...
//       <div className='container mx-auto px-10 py-16 lg:py-0 flex flex-col lg:flex-row gap-5 lg:gap-10 '>
//         <div className='relative mx-auto lg:-mt-28 lg:mx-0 w-fit min-w-60'>
//           <img
//             src={imageURL + data?.poster_path}
//             className='h-80 w-60 object-cover rounded'
//           />
//           <button onClick={() => handlePlayVideo(data)} className='mt-3 w-full py-2 px-4 text-center bg-white text-black rounded font-bold text-lg hover:bg-gradient-to-l from-red-500 to-orange-500 hover:scale-105 transition-all cursor-pointer'>Play Now</button>
//         </div>
...
const DetailsPage = () => {
  ...
  // 🔑 Force refetch every time id or explore changes
  const { data } = useFetchDetails(`/${params?.explore}/${params?.id}`, [params?.id, params?.explore])
  const { data: castData } = useFetchDetails(`/${params?.explore}/${params?.id}/credits`, [params?.id, params?.explore])
  const { data: similarData } = useFetch(`/${params?.explore}/${params?.id}/similar`, [params?.id, params?.explore])
  const { data: recommendationData } = useFetch(`/${params?.explore}/${params?.id}/recommendations`, [params?.id, params?.explore])
